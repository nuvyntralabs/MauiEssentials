...
public sealed class CliHostTests
{
    ...
    public async Task ListenStdinAcceptsAllowListedJson()
    {
        ...
        var code = await CliHost.RunAsync(
            ["listen", "--stdin", "--once", "--format", "json", "--no-update-check"],
            stdout,
            new StringWriter(),
    ...
    public async Task QueuesMissingFileIsNotInstalled()
    {
        var stdout = new StringWriter();
        var code = await CliHost.RunAsync(
            ["queues", "--format", "json", "--no-update-check"],
            stdout,
            new StringWriter(),
            new StringReader(""));
        Assert.Equal(0, code);
    ...
    public async Task AttachFromDirectorySeedsFileLanes()
    {
        ...
        try
        {
            ...
            var stdout = new StringWriter();
            var code = await CliHost.RunAsync(
                ["attach", "--from", root, "--once", "--format", "json", "--no-update-check"],
                stdout,
                new StringWriter(),
                new StringReader(""));
            Assert.Equal(0, code);
    ...
    {
        var stderr = new StringWriter();
        var code = await CliHost.RunAsync(["pull", "--no-update-check"], new StringWriter(), stderr, new StringReader(""));
        Assert.Equal(ExitCodes.Usage, code);
