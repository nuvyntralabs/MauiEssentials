...
public static class SignalParser
{
    ...
    public static ParseResult Parse(JsonElement root)
    {
        ...
        if (root.ValueKind == JsonValueKind.Array)
        {
            foreach (var item in root.EnumerateArray())
                Ingest(item, signals, presence, ref dropped);
        ...
        if (LooksLikeSignal(root))
            Ingest(root, signals, presence, ref dropped);

        return new ParseResult { Signals = signals, Presence = presence, Dropped = dropped };
    }

    static void Ingest(JsonElement item, List<PulseSignal> signals, List<PluginPresence> presence, ref int dropped)
    {
        ...
        if (!TryMaterialize(item, out var signal))
        {
            dropped++;
            return;
        }
    ...
    static bool TryMaterialize(JsonElement item, out PulseSignal signal)
    {
