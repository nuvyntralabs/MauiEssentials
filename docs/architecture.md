...
| Live Plugin.Maui.* session view | Pulse (`UseMauiPulse()` + `maui-pulse attach`) | NetworkMonitor, JobQueue, OfflineSync, Diagnostics, … (allow-list only) |
| Sibling culture `.resx` from `i18n.json` | NuvLoc (`nuvloc` + `/nuvloc.translate`) | Host UI binding (XAML / code — not NuvLoc) |
