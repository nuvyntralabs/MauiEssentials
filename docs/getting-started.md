...
- Watch a running MAUI app for allow-listed Plugin.Maui.* events → host `dotnet add package Plugin.Maui.Pulse` + `UseMauiPulse()`, CLI `dotnet tool install -g Plugin.Maui.Pulse.Cli` then `maui-pulse attach --package <id> --android|--ios --port 7878`. USB: `adb reverse tcp:7878 tcp:7878`. Integrate: [Pulse/USER-GUIDE.md](../Pulse/USER-GUIDE.md). Architecture: [Pulse/README.md](../Pulse/README.md).
- Write sibling culture `.resx` files from `i18n.json` (`AppResources.fr.resx`; `/nuvloc.status`, `/nuvloc.translate`; no API key; does not bind UI; review with a native speaker) → `NuvyntraLabs.NuvLoc.Cli` (`dotnet tool install -g NuvyntraLabs.NuvLoc.Cli --source https://api.nuget.org/v3/index.json` then `nuvloc init --configfile i18n.json --agent cursor`)
...
5. **nuget.org and GitHub Packages.** Merge the Windows-packed `net*-windows*` TFMs into the macOS `.nupkg` / `.snupkg` when those artifacts exist, then push (`--skip-duplicate`) to both feeds. nuget.org gets the nupkg and snupkg. GitHub Packages gets the nupkg at `https://nuget.pkg.github.com/nuvyntralabs/index.json`. Without the Windows merge, nuget.org shows `net10.0-windows` only as a compatibility hint. PackAsTool packages (`Plugin.Maui.*.Cli`, for example `Plugin.Maui.Performance.Cli` and `Plugin.Maui.Pulse.Cli`) publish the nupkg only. Pulse also packs the host library `Plugin.Maui.Pulse` (`UseMauiPulse()`) with the same Version.

Store the nuget.org API key as the `NUGET_KEY` Actions secret on the `nuvyntralabs` organization or on that plugin repo — never in YAML. Desktop MVVMExpress families use scoped keys: `NUGET_KEY_WPF` (`Plugin.Wpf.*`), `NUGET_KEY_WINUI` (`Plugin.WinUI.*`), `NUGET_KEY_AVALONIA` (`Plugin.Avalonia.*`), `NUGET_KEY_UNO` (`Plugin.Uno.*`). MauiDev (`Plugin.Maui.MauiDev.Cli`) uses `NUGET_KEY_MAUIDEV_CLI`. `NuvyntraLabs.UIKit` uses `NUGET_KEY_UIKIT` for nuget.org and `GITHUB_TOKEN` for GitHub Packages (linux / macos / windows pack, then merge Windows TFMs). Nuvyn (`NuvyntraLabs.Nuvyn.Cli`) uses `NUGET_KEY_NUVYN` and its own `Nuvyn/.github/workflows/ci.yml` (version alignment, NuGet check, tests, `net10.0` pack, nupkg-only publish). NuvLoc (`NuvyntraLabs.NuvLoc.Cli`) uses `NUGET_KEY_NUVLOC` and its own `NuvLoc/.github/workflows/ci.yml` (version alignment, NuGet check, tests, `net10.0` pack, nupkg-only publish). Pulse (`Plugin.Maui.Pulse` + `Plugin.Maui.Pulse.Cli`) uses `NUGET_KEY` and `Pulse/.github/workflows/ci.yml` (same fail-fast order, `net10.0` pack, CLI nupkg only, host nupkg + snupkg). NuvexaDB (`Nuventra.NuvexaDB`) uses its own `NuvexaDB/.github/workflows/ci.yml`. It packs engine **nupkg + snupkg** and PackAsTool `Nuventra.NuvexaDB.Cli` (nupkg only) and uploads them as GitHub artifacts (`NuvexaDB-NuGet`, `NuvexaDB-Cli`) next to Explorer / VS Code / Visual Studio / language SDK packs. Both VSIX packages bundle the same `nuvexa` binary. nuget.org, GitHub Packages, version alignment, and NuGet validation are commented out until multi-host publishing is decided. All families still push GitHub Packages with the job’s `GITHUB_TOKEN` (`packages: write`). Copy `.github/plugin-repo-ci.yml` when adding a new MAUI plugin repo.
