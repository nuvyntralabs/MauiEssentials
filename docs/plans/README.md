...
| NuvLoc (agent-driven localization CLI; `i18n.json`) | [nuvloc.md](nuvloc.md) | Implemented at `1.1.0` — `NuvyntraLabs.NuvLoc.Cli`, command `nuvloc`; `init --agent` + `/nuvloc.status` + `/nuvloc.translate`; sibling `.resx` (`maui` / `wpf` / `winui` / `avalonia` / `uno`); no provider / API key; resource files only |
