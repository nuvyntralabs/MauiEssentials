...
**Status:** Repository and hub submodule `WinUIMVVMExpress` exist at `1.0.0`.  
...
- [x] Create empty repo `nuvyntralabs/Plugin.WinUI.MVVMExpress` + hub submodule (no NuGet yet)

**Exit:** repo exists with `ARCHITECTURE.md` / `API-DESIGN.md` / `API-PARITY.md` copied from this plan.

### Phase 1 — Core + Host (1.0.0)
...
- [x] `dotnet new winui-mvvmexpress` / `winui-mvvmexpress-page`
- [x] VS Code / Visual Studio extensions that only install the template pack and run `dotnet new`
- [x] Getting started, `llms.txt`, `AGENTS.md`
- [x] Hub catalog row, `docs/packages/README.md`, skill catalog
- [x] CI: Linux pack Core/Validation/Pagination/Testing/Templates; Windows pack Host/Navigation/Dialogs; nuget.org via `NUGET_KEY_WINUI`; GitHub Packages via `GITHUB_TOKEN`
