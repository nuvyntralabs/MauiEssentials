...
| [Plugin.Maui.HttpForge](https://github.com/nuvyntralabs/Plugin.Maui.HttpForge) | Source-generated REST client (Refit-style interfaces; query objects, streaming, optional Testing/Newtonsoft/XML) (Android, iOS, Mac Catalyst, Windows) | [NuGet](https://www.nuget.org/packages/Plugin.Maui.HttpForge) |
...
| [Plugin.Maui.Performance](https://github.com/nuvyntralabs/Plugin.Maui.Performance) | Lightweight profiler for startup, pages, APIs, images, memory, and `maui profile` (`maui-perf` has the same 4-hour nuget.org update check as `maui-dev`) | [NuGet](https://www.nuget.org/packages/Plugin.Maui.Performance) |
...
| [NuvLoc](NuvLoc/) (`nuvloc`) | Writes sibling culture `.resx` files from `i18n.json` (`AppResources.es.resx`, …). `/nuvloc.status` + `/nuvloc.translate`. No API key. Does not bind UI. Does not guarantee translation quality — review with a native speaker before ship. | `dotnet tool install -g NuvyntraLabs.NuvLoc.Cli --source https://api.nuget.org/v3/index.json` |
...
[NuvexaDB](https://github.com/nuvyntralabs/NuvexaDB) (`Nuventra.NuvexaDB`) is a hub module: a standalone embedded document database for .NET and MAUI (one encrypted `.nvx` file, Avalonia Explorer, `nuvexa` CLI as `Nuventra.NuvexaDB.Cli`, VS / VS Code editors that bundle the same CLI). It is not a `Plugin.Maui.*` package. JobQueue and OfflineSync remain the SQLite-backed tools for durable jobs and sync.

[NuvyntraLabs.UIKit](https://github.com/nuvyntralabs/NuvyntraLabs.UIKit) (`NuvyntraLabs.UIKit`) is a hub module: Lumina-themed MAUI controls and page recipes (`NVCheckBox`, `NVRadioButton`, …). It is not a `Plugin.Maui.*` package. Version `1.6.0` is 201 controls + 66 recipes with a richer Lumina chrome (lifted fills, hairline, glow, recessed fields). `NVInputField` replaces `NVEmailField`. Phase 7 harden (type scale, RTL, contrast, overlay keys) stays. [NuGet](https://www.nuget.org/packages/NuvyntraLabs.UIKit) · [Docs](https://nuvyntralabs.github.io/packages/nuvyntralabs-uikit/). Publishing stays pipeline-only.
