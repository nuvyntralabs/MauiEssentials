...
Existing apps stay on the component path for packages. `nuvyn adopt` attaches the slash chain without changing MVVM / UI / HTTP. `nuvyn update` refreshes skills only. Do not re-run `nuvyn init` on a tree that already exists.
...
1. Decide the door. New Nuvyntra host / “whole ecosystem” / “build me a MAUI app on this stack” → Nuvyn (`nuvyn init`) only. Existing MAUI app + spec chain without a stack rewrite → `nuvyn adopt`. Existing app + one control or capability → one component. Do not require Nuvyn for a component install.
...
- ScreenGuard on iOS is a capture overlay (`CaptureStateChanged`; host supplies the view), not a screenshot block. LocalNotifications does not register FCM / APNs tokens. Geofence Android uses Play Services `GeofencingClient` and persists regions across process death (`Raise()` remains for samples). BluetoothSerial is classic SPP (Android first-class; iOS MFi only), not BLE UART. VideoPipeline probes duration, writes a thumbnail, and tries an OS transcode when over budget (no FFmpeg; `CannotTranscode` if the device cannot encode). TlsPin is fail-closed and needs a backup pin. AppReview Android uses Play Core `ReviewManager` on Play-installed builds (`Unavailable` otherwise). LocalStore copies `.db` ↔ `.nvx` (and any other engine pair) when `AutoMigrate` and `Map<T>` are set.
...
├── NuvLoc/              → NuvyntraLabs.NuvLoc.Cli (`nuvloc` sibling `.resx` from `i18n.json`; `/nuvloc.status` + `/nuvloc.translate`)
...
For an **existing** MAUI app that should keep its stack, use `nuvyn adopt` (workflow + `adopt-report.md` only). Do not run `nuvyn init` on that tree.

Locked stack (greenfield `init` only): `Plugin.Maui.MVVMExpress` + `NuvyntraLabs.UIKit` + the smallest `Plugin.Maui.*` set.
